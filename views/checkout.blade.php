        ...
        <div class="container px-6 mx-auto">
            <div class="flex justify-center my-6">
                <div class="flex flex-col w-full p-8 text-gray-800 bg-white shadow-lg pin-r pin-y md:w-4/5 lg:w-4/5">
                    ...
                    @endif
                    <h3 class="text-3xl font-bold">Carts No : {{ $cartUser}}</h3>
                    <div class="flex-1">
                                ...
                                @foreach ($cartItems as $item)
                                    <tr>
                                        ...
                                        <td class="hidden text-right md:table-cell">
                                            <p class="mb-2 text-gray-900 font-bold">{{ $item->quantity }}</p>
                                        </td>
                                        <td class="hidden text-right md:table-cell">
                                            <p class="mb-2 text-gray-900 font-bold">{{ $item->price }}</p>
                                        </td>
                                        <td class="hidden text-right md:table-cell">
                                            <p class="mb-2 text-gray-900 font-bold">{{ $item->quantity * $item->price }}</p>
                                        </td>
                                @endforeach
                            </tbody>
                        </table>
                        <div class="flex justify-between items-center my-5">
                            <div class="font-semibold text-2xl text-red-600">Total: ${{ Cart::getTotal() }}</div>
                            <div>
